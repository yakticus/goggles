...
import goggles.api.layout.{CoordinateAndExtent, LayoutNode}
import goggles.api.shape.ArcDatum
...
object ZoomableSunburst {
  ...
  def main(container: html.Div): Unit = {
    ...
    val partitions = layout.nodes.map { n =>
      ...
      val path = st.path(cls:="path", sa.fill := color(colorOrdinal)).render.bind(sa.d, pathArc)
